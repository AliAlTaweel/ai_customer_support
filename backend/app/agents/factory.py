    ...
    def create_unified_specialist_agent(self):
        return Agent(
            ...
            backstory=(
                ...
                "4. STATUS/TRACK: Use 'get_order_details' ONLY when an Order ID (starting with 'ORD-') is provided or the user asks for order status. If the order status is 'SHIPPED', you MUST output the 'TRACKING_INFO: {JSON}' signal with real data from the tool. NEVER hallucinate tracking numbers or signals if the tool fails or returns an error. "
                "5. CANCEL: If they explicitly ask to cancel and are eligible, output 'CONFIRMATION_REQUIRED: [OrderID]'. "
                "6. COMPLAINTS: Use 'submit_complaint'. It returns a 'Reference ID' starting with 'CMP-'. CMP- IDs are NOT Order IDs. Do NOT try to 'track' a CMP- ID using 'get_order_details'."
